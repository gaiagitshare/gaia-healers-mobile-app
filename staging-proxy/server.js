...
async function eventDetail(req, res, origin, eventId) {
  const [event, agenda, speakers, exhibitors, sponsors, announcements, venueMap] = await Promise.all([
...
function fallbackAssistReply(prompt, intent = '') {
  ...
  if (normalized.includes('chakra') || normalized.includes('wellness') || normalized.includes('energy') || normalized.includes('chart') || normalized.includes('colour') || normalized.includes('color')) {
    return 'Energy Studio has three separate tools: Energy check for today’s body point and practice, Horoscope for reflective daily guidance, and Chakra match for the seven-centre guide. The five-question Colour Test now lives in Energy, alongside Numerology, Today\u2019s Sky and Bio-Well.';
  }
