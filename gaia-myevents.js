...
(function () {
  ...
  function ticketStatus(ticket) {
    var rs = (ticket.registrationStatus || 'registered').toLowerCase();
    if (ticket.valid === false || rs === 'refunded' || rs === 'revoked' || rs === 'cancelled') {
      return { text: (rs === 'refunded' ? 'Refunded' : rs === 'revoked' ? 'Revoked' : rs === 'cancelled' ? 'Cancelled' : 'Not valid') + ' \u2014 not valid for entry', tone: 'bad' };
    }
  ...
  function ticketHtml(data) {
    ...
    return '<div class="g-ticket__panel" role="dialog" aria-label="My ticket" aria-modal="true">'
      ...
      + (ticket.valid === false ? '<p class="g3-void" style="margin:6px 0 0;color:#b3341f;font-weight:600;">This ticket is not valid for entry.</p>' : '')
